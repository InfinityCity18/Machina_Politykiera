...
impl MemoryScanner {
    ...
    pub fn addresses_and_values(&self) -> Result<Vec<(MemoryAddress, Vec<u8>)>, Box<dyn Error>> {
    ...
    fn kmp(file: &File, val: &ScanValue, len_of_memory: usize, process: Rc<Process>, start_offset: usize) -> Result<Vec<MemoryAddress>,Box<dyn Error>> {
    ...
    pub fn next_scan(&mut self, scan_settings: ScanSettings) -> Result<(), Box<dyn Error>> {
        let process = scan_settings.process();
        if process.pid() != self.matching_addresses.get(0).ok_or("No addresses in list")?.process.pid() {
...
use super::MemoryScanner;
    ...
    fn test_kmp() -> Result<(), Box<dyn std::error::Error>>{
        ...
        let addresses = MemoryScanner::kmp(&file, &ScanValue::String("hello world".to_string()), 110, Rc::new(Process::myself()?), 0)?;
