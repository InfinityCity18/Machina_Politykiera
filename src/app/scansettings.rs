...
mod tests {
    ...
    fn test_usr_input_byte() -> Result<(), Box<dyn Error>>{
        ...
        assert_eq!(ScanValue::from_user_input("12".to_string(), Byte)?, ScanValue::Byte(12));
        Ok(())
    }
    #[test]
    ...
    fn test_usr_input_float() {
        assert_eq!(ScanValue::from_user_input("1.22143".to_string(), Float).unwrap(), ScanValue::Float(1.22143));
    }

    #[test]
    fn test_usr_input_string() {
        assert_eq!(ScanValue::from_user_input("hello world".to_string(), String("hello world".len())).unwrap(), ScanValue::String("hello world".to_string()));
    }
    ...
    fn test_scanval_from_bytes() {
        assert_eq!(ScanValue::convert_from_bytes(&['a' as u8,'b' as u8,'c' as u8], String(5)).unwrap(), ScanValue::String("abc".to_string()));
    }

    #[test]
    fn test_scanval_from_bytes_err_utf8() {
        assert!(ScanValue::convert_from_bytes(&['a' as u8,'b' as u8,211 as u8], String(5)).is_err());
    }
